...
public class BfsMan {
	...
	public BfsMan(int arrInicio[][], int arrMeta[][]) {
	...
	public void buscarSolucion() {
		...
		while (!colaAbiertos.isEmpty()) {
			nodoActual = colaAbiertos.poll().toString(); // recuperamos el
															// primero nodo
			if (nodoActual.equals(estadoFinal)) { // verificamos si estado
													// actual es igual al final
				solucion = true;
		...
		System.out.println("Tiempo de ejecucion " + tiempoFinal * 0.001
				+ " segundos");
	}// fin de hacerBusqueda

	private void agregarNodoACola(String nodoHijo, String nodoPadre) {
		if (!nivelProfundidad_map.containsKey(nodoHijo)) { // Verificamos que no
															// haya nodos
															// repetidos
			profundidad = (nodoPadre == null) ? 0 : nivelProfundidad_map
					.get(nodoPadre) + 1; // incrementamos en 1 la profundidad
			nodosSinRepeticion++;
			nivelProfundidad_map.put(nodoHijo, profundidad);
			heuristica = calcularManhattan(nodoHijo, estadoFinal) + profundidad; // calculamos
																					// la
																					// heuristica
																					// para
																					// el
																					// nuevo
																					// estado
			colaAbiertos.add(new OrdenarEstado(heuristica, nodoHijo)); // agregamos
																		// a la
																		// cola
																		// de
																		// prioridades
	...
	public void crearHijos(String nodoPadre) {
		...
		while (cero != 2 && cero != 5 && cero != 8) {
			String nodoHijo = nodoPadre.substring(0, cero)
					+ nodoPadre.substring(cero + 1, cero + 2) // numero en
																// movimiento
					+ "0" + nodoPadre.substring(cero + 2);
		...
		while (cero != 0 && cero != 3 && cero != 6) {
			String nodoHijo = nodoPadre.substring(0, cero - 1) + "0"
					+ nodoPadre.substring(cero - 1, cero) // numero en
															// movimiento
		...
		while (cero != 6 && cero != 7 && cero != 8) {
			String nodoHijo = nodoPadre.substring(0, cero)
					+ nodoPadre.substring(cero + 3, cero + 4) // numero en
																// movimiento
					+ nodoPadre.substring(cero + 1, cero + 3) + "0"
		...
		while (cero != 0 && cero != 1 && cero != 2) {
			String nodoHijo = nodoPadre.substring(0, cero - 3) + "0"
					+ nodoPadre.substring(cero - 2, cero)
					+ nodoPadre.substring(cero - 3, cero - 2) // numero en
																// movimiento
					+ nodoPadre.substring(cero + 1);
			if (nodoPadre.compareTo(nodoHijo) != 0) {
				agregarNodoACola(nodoHijo, nodoPadre);
	...
	public void imprimirSolucion(String nodoActual) {
		...
		while (trazarCamino != null) {
			System.out.println("********************************");
			System.out.println(trazarCamino + " en "
					+ nivelProfundidad_map.get(trazarCamino));
			int cero = trazarCamino.indexOf("0");
			System.out.println("el cero esta en la posicion " + cero);
			System.out.println("Valor Manhattan: "
					+ calcularManhattan(trazarCamino, estadoFinal));
			for (int i = 0; i < 9; i++) {
				System.out.print(String.valueOf(trazarCamino.charAt(i)) + " "); // convertimos
																				// de
																				// Integer
																				// a
																				// String
				if ((i + 1) % 3 == 0) {
					System.out.println("");// salto de linea para visualizar
											// string como matriz
				}
			}
			...
			if (trazarCamino != null) {
				int xy = Integer.parseInt(String.valueOf(trazarCamino
						.charAt(cero)));
	...
	public int calcularManhattan(String siguienteNodo, String estadoMeta) {
		// tabla para la distancia manhattan
		int[][] valoresManhattan = { { 0, 1, 2, 1, 2, 3, 2, 3, 4 }, // 0
				{ 1, 0, 1, 2, 1, 2, 3, 2, 3 }, // 1
				{ 2, 1, 0, 3, 2, 1, 4, 3, 2 }, // 2
				{ 1, 2, 3, 0, 1, 2, 1, 2, 3 }, // 3
				{ 2, 1, 2, 1, 0, 1, 2, 1, 2 }, // 4
		...
		int resultado = 0;
		for (int i = 1; i < 9; i++) {
			int xi = siguienteNodo.indexOf(String.valueOf(i));
			int yi = estadoMeta.indexOf(String.valueOf(i));
			h = valoresManhattan[xi][yi];
			resultado = resultado + h;
		}
		// System.out.println("Heuristica "+resultado);
		return resultado;

	}// fin de calcularManhattan
